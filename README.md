...
言語設定の役割：
- **日本語（ja）**: [VOICE]タグのサマリーを日本語で書く
- **英語（en）**: [VOICE]タグのサマリーを英語で書く

**重要: Claudeの応答言語とは独立した設定です**

| 設定 | 場所 | 目的 |
|------|------|------|
| `language` | settings.json | Claudeの応答言語（会話全体） |
| `VOICE_LANG` | ~/.cvi/config | [VOICE]タグの言語（読み上げ用） |

**ユースケース例**:
- Claudeと日本語で会話（`language: "japanese"`）
- タスク完了通知は英語で聞く（`VOICE_LANG=en`）
- 英語学習やリスニング練習に活用
